    ...
    return (
        ...
        <div className="absolute top-16 right-2  z-[2000] sm:w-52 w-40">
            <div className="relative  w-full border rounded-lg shadow bg-gray-800 border-gray-500">
                ...
                <div className="flex flex-col items-center sm:pb-10 pb-4 mt-5">
                    <img className="sm:w-24 w-10 sm:h-24 h-10 mb-3 rounded-full shadow-lg"
                        src={process.env.REACT_APP_SERVER_URL + auth.img} alt="" />
                    <h5 className="mb-1 sm:text-xl text-base sm:font-medium font-normal  text-white">{auth.name}</h5>
                    <span className="sm:text-sm text-xs  text-gray-400">{auth.email}</span>

                </div>
                <div className="flex justify-end px-4 mb-3">
                    <div id="dropdown" className="z-10  sm:text-base text-xs list-none  divide-y divide-gray-100 rounded-lg shadow w-44 bg-gray-700">
